...
const Testimonial = () => {
  ...
  return (
    <section id="testimonial" className="py-32 relative overflow-hidden">
      <div className="absolute top-1/2 left-1/4 w-[880px] h-[880px] bg-primary/5 rounded-full blur-3xl -translatex-1/2 -translate-y-1/2" />
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative">
            {/* Main Quote */}
            <div className="glass p-8 rounded-3xl md:p-12 glow-border animate-fade-in animation-delay-300">
              <div className="absolute -top-4 left-8 w-12 h-12 rounded-full bg-primary flex items-center justify-center">
                <Quote className="w-5 h-5 text-primary-foreground" />
              </div>
              <blockquote className="text-xl md:text-2xl font-medium pt-4 mb-8 leading-relaxed">
                "{testimonials[activeIdx].quote}"
              </blockquote>
              <div className="flex items-center gap-4">
                <img
                  src={testimonials[activeIdx].avatar}
                  alt={testimonials[activeIdx].avatar}
            ...
            <div className="flex items-center justify-center gap-4 mt-8">
              <button
                className="p-3 rounded-full glass hover:bg-primary/10 hover:text-primary transition-all animate-fade-in animation-delay-200"
                onClick={previous}
              >
                <ChevronLeft />
              </button>

              {/* testimonil changing dots */}
              <div className="flex gap-2">
                {testimonials.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setActiveIdx(i)}
                    className={`w-2 h-2 transition-all duration-300 rounded-full ${i === activeIdx ? "bg-primary w-8 rounded-full" : "bg-muted-foreground/30 hover:bg-primary/50"}`}
                  />
                ))}
              </div>
